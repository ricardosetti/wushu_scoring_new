        ...
        <form @submit.prevent="handleSubmit">
          <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <div class="mb-2">
              ...
              <select v-model="newParticipant.school_id" class="border p-2 w-full rounded">
            ...
            <div class="mb-2">
              ...
              <input v-model="newParticipant.birthdate" type="date" class="border p-2 w-full rounded" />
            </div>
            <div class="mb-2">
            ...
            <div class="mb-2">
              ...
              <select v-model="newParticipant.gender" class="border p-2 w-full rounded">
...
export default {
  ...
  computed: {
    availableDivisions() {
      const currentDivisionIds = this.participantDivisions.map(div => div.id);
      return this.allDivisions.filter(div => !currentDivisionIds.includes(div.id));
    },
  },
  mounted() {
  ...
  methods: {
    ...
    handleSubmit() {
      this.errorMessage = '';
      console.log('Submitting form, editParticipantId:', this.editParticipantId, 'Data:', this.newParticipant);
    ...
    async addParticipant() {
      console.log('Adding new participant, Data:', this.newParticipant);
      try {
        ...
        this.errorMessage = error.response?.data?.error || 'Failed to add participant. Please try again.';
      }
    },
    ...
    async updateParticipant() {
      console.log('Updating participant with id:', this.editParticipantId, 'Data:', this.newParticipant);
      try {
        ...
        this.errorMessage = error.response?.data?.error || 'Failed to update participant. Please try again.';
      }
    },
    ...
    async manageDivisions(participant) {
      this.selectedParticipant = participant;
      this.showDivisionForm = true;
      try {
        const response = await axios.get(
          `http://${process.env.VITE_SERVER_HOST}:${process.env.VITE_SERVER_PORT}/participants/${participant.id}/divisions`
        );
        this.participantDivisions = response.data;
      } catch (error) {
        console.error('Error fetching participant divisions:', error.response ? error.response.data : error.message);
        this.participantDivisions = [];
      }
    },
    async addDivisionToParticipant() {
      if (!this.selectedDivisionId) return;
      try {
        await axios.post(
          `http://${process.env.VITE_SERVER_HOST}:${process.env.VITE_SERVER_PORT}/participants/division`,
          {
        ...
        await this.fetchParticipants();
        const response = await axios.get(
          `http://${process.env.VITE_SERVER_HOST}:${process.env.VITE_SERVER_PORT}/participants/${this.selectedParticipant.id}/divisions`
        );
        this.participantDivisions = response.data;
    ...
    async removeDivisionFromParticipant(divisionId) {
      try {
        await axios.delete(
          `http://${process.env.VITE_SERVER_HOST}:${process.env.VITE_SERVER_PORT}/participants/division`,
          {
            data: { participant_id: this.selectedParticipant.id, division_id: divisionId },
          }
        );
        await this.fetchParticipants();
        this.participantDivisions = this.participantDivisions.filter(div => div.id !== divisionId);
      } catch (error) {
